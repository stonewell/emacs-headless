...
(declare-function headless-frame-geometry "headlessfns.c" (&optional frame))
...
`internal-border-width' is the width of the internal border of
  ...
  (let* ((frame (window-normalize-frame frame))
    ...
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-frame-geometry frame))
     (t
      (list
       '(outer-position 0 . 0)
...
(declare-function headless-frame-edges "headlessfns.c" (&optional frame type))
...
FRAME."
  (let* ((frame (window-normalize-frame frame))
    ...
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-frame-edges frame type))
     (t
...
(declare-function headless-mouse-absolute-pixel-position "headlessfns.c")
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-mouse-absolute-pixel-position))
     (t
      (cons 0 0)))))
...
(declare-function headless-set-mouse-absolute-pixel-position "headlessfns.c" (x y))
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-set-mouse-absolute-pixel-position x y)))))
...
(declare-function headless-frame-list-z-order "headlessfns.c" (&optional display))
...
Return nil if DISPLAY contains no Emacs frame."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-frame-list-z-order display)))))
...
(declare-function headless-frame-restack "headlessfns.c" (frame1 frame2 &optional above))
  ...
  (if (and (frame-live-p frame1)
      ...
      (let ((frame-type (framep-on-display frame1)))
        (cond
         ...
         ((eq frame-type 'headless)
          (headless-frame-restack frame1 frame2 above))
         ((eq frame-type 'pgtk)
...
frame's display)."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((memq frame-type '(x ns haiku pgtk headless))
...
DISPLAY can be a display name, a frame, or nil (meaning the selected
frame's display)."
  (not (null (memq (framep-on-display display) '(x w32 ns pgtk haiku headless)))))
...
If DISPLAY is omitted or nil, it defaults to the selected frame's display."
  (let ((frame-type (framep-on-display display)))
    (cond
     ((memq frame-type '(x w32 ns haiku headless pgtk))
...
monitor, use `display-monitor-attributes-list'."
  (and (memq (framep-on-display display) '(x w32 ns haiku headless pgtk))
...
(declare-function haiku-display-monitor-attributes-list "haikufns.c"
		  (&optional terminal))
(declare-function headless-display-monitor-attributes-list "headlessfns.c"
		  (&optional terminal))
...
monitors."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'headless)
      (headless-display-monitor-attributes-list display))
     (t
